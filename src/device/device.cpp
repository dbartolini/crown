...
int Device::main_loop()
{
	...
	_resource_manager->register_type(RESOURCE_TYPE_SHADER,           RESOURCE_VERSION_SHADER,           NULL,                            NULL,                              shader_resource_internal::online,   shader_resource_internal::offline);
	...
	_pipeline->create(_width, _height, merged_render_settings(this), _options._export);
...
void Device::refresh(const char *json)
{
	...
	for (u32 i = 0; i < array::size(list); ++i) {
		...
		bool is_type_reloadable = resource_type == RESOURCE_TYPE_SCRIPT
			...
			|| resource_type == RESOURCE_TYPE_MESH_ANIMATION
		...
		if (is_type_reloadable && _resource_manager->can_get(resource_type, resource_name)) {
			...
			if (resource_type == RESOURCE_TYPE_SCRIPT) {
				...
				{
					World *w = (World *)container_of(cur, World, _node);
					mesh_animation_player::reload(*w->_mesh_animation_player, (const MeshAnimationResource *)old_resource, (const MeshAnimationResource *)new_resource);
					w->_animation_state_machine->reload((const MeshAnimationResource *)old_resource, (const MeshAnimationResource *)new_resource);
				}
			...
			} else if (resource_type == RESOURCE_TYPE_SPRITE_ANIMATION) {
				ListNode *cur;
				list_for_each(cur, &_worlds)
				{
					World *w = (World *)container_of(cur, World, _node);
