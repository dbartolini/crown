...
namespace LevelEditorApi
{
	...
	public string add_light_component(Guid id, Guid component_id, string type, double range, double intensity, double spot_angle, Vector3 color, double shadow_bias)
	{
		return "LevelEditor:add_light_component(\"%s\", \"%s\", \"%s\", %.17g, %.17g, %.17g, %s, %.17g)".printf(id.to_string()
			, component_id.to_string()
			, type
			, range
			, intensity
	...
	public string set_light(Guid id, string type, double range, double intensity, double spot_angle, Vector3 color, double shadow_bias)
	{
		return "LevelEditor._objects[\"%s\"]:set_light(\"%s\", %.17g, %.17g, %.17g, %s, %.17g)".printf(id.to_string()
			, type
			, range
			, intensity
			, spot_angle
