/*
 * Copyright (c) 2012-2026 Daniele Bartolini et al.
 * SPDX-License-Identifier: GPL-3.0-or-later
 */

namespace Crown
{
public class Toolbar : Gtk.Box
{
	public Toolbar()
	{
		this.orientation = Gtk.Orientation.VERTICAL;
		this.halign = Gtk.Align.START;
		this.valign = Gtk.Align.START;
		this.margin_top = 8;
		this.margin_start = 8;
		this.add_css_class("toolbar");

		add_tool_buttons();
		add_snap_buttons();
		add_reference_system_buttons();
		add_snap_to_grid_buttons();
	}

	public void add_tool_buttons()
	{
		this.append(make_toggle_button("app.tool", new GLib.Variant.int32(ToolType.PLACE), "tool-place"));
		this.append(make_toggle_button("app.tool", new GLib.Variant.int32(ToolType.MOVE), "tool-move"));
		this.append(make_toggle_button("app.tool", new GLib.Variant.int32(ToolType.ROTATE), "tool-rotate"));
		var last = make_toggle_button("app.tool", new GLib.Variant.int32(ToolType.SCALE), "tool-scale");
		last.margin_bottom = last.margin_bottom + 8;
		this.append(last);
	}

	public void add_snap_buttons()
	{
		this.append(make_toggle_button("app.snap", new GLib.Variant.int32(SnapMode.RELATIVE), "reference-local"));
		var last = make_toggle_button("app.snap", new GLib.Variant.int32(SnapMode.ABSOLUTE), "reference-world");
		last.margin_bottom = last.margin_bottom + 8;
		this.append(last);
	}

	public void add_reference_system_buttons()
	{
		this.append(make_toggle_button("app.reference-system", new GLib.Variant.int32(ReferenceSystem.LOCAL), "axis-local"));
		var last = make_toggle_button("app.reference-system", new GLib.Variant.int32(ReferenceSystem.WORLD), "axis-world");
		last.margin_bottom = last.margin_bottom + 8;
		this.append(last);
	}

	public void add_snap_to_grid_buttons()
	{
		var last = make_toggle_button("app.snap-to-grid", null, "snap-to-grid");
		last.margin_bottom = last.margin_bottom + 8;
		this.append(last);
	}

	public Gtk.ToggleButton make_toggle_button(string action_name
		, GLib.Variant? action_target
		, string icon_name
		)
	{
		var btn = new Gtk.ToggleButton();
		btn.action_name = action_name;
		if (action_target != null)
			btn.action_target = action_target;
		btn.focusable = false;
		btn.add_css_class("flat");
		btn.add_css_class("image-button");

		var img = new Gtk.Image.from_icon_name(icon_name);
		img.margin_bottom
			= img.margin_end
			= img.margin_start
			= img.margin_top
			= 8
			;
		btn.set_child(img);

		return btn;
	}
}

} /* namespace Crown */
