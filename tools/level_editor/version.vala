...
public struct Version
{
	...
	public static async bool load_versions(out Version remote)
	{
		remote = {};
		...
		try {
			...
			logi ("Got %zu bytes".printf(json.length));
			logi ("crown %s".printf(crown));
			remote = Version.parse(crown);
			return true;
		} catch (Error e) {
			warning ("Failed to download %s: %s", file.get_uri(), e.message);
		}
